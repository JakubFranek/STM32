...
void BME280_read_trim_coeffs(void) {
	uint8_t trimdata[32];
	...
	HAL_I2C_Mem_Read(BME280_I2C, BME280_ADDRESS, 0xE1, 1,
			(uint8_t*) trimdata + 25, 7, HAL_MAX_DELAY);
...
int BME280_read_raw_data(void) {
	...
	if (chipID == 0x60) {
		...
		HAL_I2C_Mem_Read(BME280_I2C, BME280_ADDRESS, PRESS_MSB_REG, 1, RawData,
				8, HAL_MAX_DELAY);
 ...
 */
void BME280_set_forced_mode(void) {
	uint8_t datatowrite = 0;
 ...
 */
void BME280_Measure_TPH(float *temperature, float *pressure, float *humidity) {
	if (BME280_read_raw_data() == 0) {
		if (tRaw == 0x800000)
		...
		if (pRaw == 0x800000)
			*pressure = 0; // value in case pressure measurement was disabled
		else {
#if SUPPORT_64BIT
		...
		if (hRaw == 0x8000)
			*humidity = 0; // value in case humidity measurement was disabled
		else {
...
void BME280_Measure_TH(float *temperature, float *humidity) {
	if (BME280_read_raw_data() == 0) {
		if (tRaw == 0x800000)
			*temperature = 0; // value in case temp measurement was disabled
		else {
