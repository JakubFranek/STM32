...
static uint64_t read_register_multibyte(uint8_t reg, uint8_t num_bytes)
{
    uint8_t command = NRF24L01P_CMD_R_REGISTER | reg;
    uint8_t status;
    uint8_t read_val[num_bytes];
    uint64_t register_val = 0;

    cs_low();

    HAL_SPI_TransmitReceive(NRF24L01P_SPI, &command, &status, 1, 2000);
    HAL_SPI_Receive(NRF24L01P_SPI, read_val, num_bytes, 2000);

	cs_high();

	for(int i = 0; i < num_bytes; i++)
	{
		register_val |= read_val[i] << i;
	}
...
void nrf24l01p_rx_init(channel MHz, air_data_rate bps)
{
    ...
    nrf24l01p_rx_set_payload_length(NRF24L01P_PAYLOAD_LENGTH);
...
void nrf24l01p_tx_transmit_from_powerdown(uint8_t* tx_payload)
{
	nrf24l01p_power_up();
	HAL_Delay(2);
	ce_high();
...
void debug_read_all_regs()
{
	...
	uint64_t rx_addr_p0 = read_register_multibyte(NRF24L01P_REG_RX_ADDR_P0, 5);
	uint64_t rx_addr_p1 = read_register_multibyte(NRF24L01P_REG_RX_ADDR_P1, 5);
	uint8_t rx_addr_p2 = read_register(NRF24L01P_REG_RX_ADDR_P2);
	uint8_t rx_addr_p3 = read_register(NRF24L01P_REG_RX_ADDR_P3);
	uint8_t rx_addr_p4 = read_register(NRF24L01P_REG_RX_ADDR_P4);
	uint8_t rx_addr_p5 = read_register(NRF24L01P_REG_RX_ADDR_P5);
	uint8_t tx_addr = read_register(NRF24L01P_REG_TX_ADDR);
...
void nrf24l01p_rx_set_payload_length(widths bytes)
{
...
void nrf24l01p_set_rf_data_rate(air_data_rate bps)
{
    // Set value to 0
